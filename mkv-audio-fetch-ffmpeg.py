...
# Analyze volume levels of audio tracks using ffmpeg in a file
def analyze_audio_volume_file(file):
    try:
        probe = ffmpeg.probe(file)
        audio_tracks = [s for s in probe['streams'] if s['codec_type'] == 'audio']
        audio_tracks_volumes = []
        for idx, track in enumerate(audio_tracks):
            print(f"Analyzing volume for Track {track["index"]}: Language = {track['tags']['language'] if 'tags' in track and 'language' in track['tags'] else 'und'}")
            #test = ffmpeg.input(file)[f'a:{idx}'].filter('volumedetect').output('-', format='null').run(capture_stdout=True, capture_stderr=True)
            stdout_str = ffmpeg.input(file)[f'{track["index"]}'].filter('volumedetect').output('-', format='null').run(capture_stdout=True, capture_stderr=True)
            #ffmpeg.input(file, hide_banner=True, map=f'0:{track["index"]}').filter('volumedetect').output('-', format='null').run(capture_stdout=True, capture_stderr=True)
